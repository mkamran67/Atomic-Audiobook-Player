...
import { myBooks, popularAuthors, keepReading, stats } from '../../mocks/books';
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-purple-100 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 transition-colors duration-300">
      <Sidebar
        ...
        playerBarVisible={playerCollapsed}
      />

      <main className={`flex-1 transition-all duration-300 px-4 md:px-8 pt-20 md:pt-8 ${playerCollapsed ? 'pb-52' : 'pb-8'} ${sidebarCollapsed ? 'md:ml-20' : 'md:ml-52'} ${playerCollapsed ? 'md:mr-0' : 'md:mr-[400px]'} min-w-0`}>
          ...
          <div className="-mx-4 md:-mx-8 flex gap-6 overflow-x-auto pb-4 scrollbar-styled px-4 md:px-8">
            {myBooks.map((book) => (
              <BookCard
                ...
                onClick={() => dispatch(setCurrentTrack(keepReading[book.id - 1]))}
              />
            ))}
          </div>
        </section>
            ...
            <div className="space-y-3">
              {keepReading.map((book) => {
                ...
                return (
                  <div
                    key={book.id}
                    onClick={() => dispatch(setCurrentTrack(book))}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-500 dark:text-gray-500 whitespace-nowrap flex-shrink-0">
                          Ch {prog?.currentChapter ?? book.currentChapter}/{prog?.totalChapters ?? book.totalChapters}
                        </span>
                      </div>
                    </div>
                    <span className="text-xs text-gray-500 dark:text-gray-500 flex-shrink-0 hidden sm:block">{book.currentPage}</span>
                  </div>
                );
              })}
            </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full overflow-hidden">
                      <img src={author.avatar} alt={author.name} className="w-full h-full object-cover object-top" />
                    </div>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 md:p-6 hover:shadow-lg dark:hover:shadow-purple-900/20 transition-all">
            ...
            <p className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white mb-1">{stats.readBooks}</p>
            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Read Books</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 md:p-6 hover:shadow-lg dark:hover:shadow-purple-900/20 transition-all">
            ...
            <p className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white mb-1">{stats.readAuthors}</p>
            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Read Authors</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 md:p-6 hover:shadow-lg dark:hover:shadow-purple-900/20 transition-all">
            ...
            <p className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white mb-1">{stats.areRead}</p>
            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Are Read</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 md:p-6 hover:shadow-lg dark:hover:shadow-purple-900/20 transition-all">
            ...
            <p className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white mb-1">{stats.bookPages}</p>
            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">Book Pages</p>
          </div>
        </section>
      </main>
