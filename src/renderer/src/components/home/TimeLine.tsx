...
export default function TimeLine() {
  ...
  return (
    <div className="w-full h-full p-4">
      <h3 className="text-2xl text-gray-300 mb-4">Recent Activity</h3>
      <ul className="space-y-3">
        {stats.slice(0, 8).map((stat) => (
          <li key={stat.bookTitle} className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-800/50 transition-colors">
            <ClockIcon className="w-5 h-5 text-gray-500 shrink-0" />
            <div className="min-w-0">
              <p className="text-sm font-medium text-white truncate">{stat.bookTitle}</p>
